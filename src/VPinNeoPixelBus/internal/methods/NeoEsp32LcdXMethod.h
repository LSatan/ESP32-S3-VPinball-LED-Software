...
public:
	...
	void Destruct()
    {
        ...
        // 1. Hardware-Modul deaktivieren
        periph_module_disable(PERIPH_LCD_CAM_MODULE);
        periph_module_reset(PERIPH_LCD_CAM_MODULE);

        // 2. DMA-Kanal zurücksetzen
        gdma_reset(_dmaChannel);

        // --- DER FIX: TREIBER SAUBER TRENNEN UND LÖSCHEN ---
        // Löst die Verbindung zu Peripheral 5 (LCD_CAM) auf
        gdma_disconnect(_dmaChannel); 
        // Gibt den reservierten DMA-Kanal wieder an das Betriebssystem zurück
        gdma_del_channel(_dmaChannel); 
        // ---------------------------------------------------

        // 3. RAM wieder freigeben
